...
FILTER_CHOICE_SORT_UP = "10"
FILTER_CHOICE_SORT_DOWN = "11"
FILTER_CHOICE_LARGER_THAN = "12"
FILTER_CHOICE_SMALLER_THAN = "13"
##########################
# Export filters grouped #
##########################


# Number fields(Interger & Float)
NUMBER_FILTER_CHOICES = (
    ("", _("Nothing")),
    (FILTER_CHOICE_LARGER_THAN, _("Larger than")),
    (FILTER_CHOICE_SMALLER_THAN, _("Smaller than")),
    (FILTER_CHOICE_SORT_UP, _("Rank from lowest to highest ")),
    (FILTER_CHOICE_SORT_DOWN, _("Rank from highest to lowest ")),
...
def tryLarger(val, field):
    try:
        return int(float(val)) <= int(float(field))
    except:
        return True

def trySmaller(val, field):
    try:
        return int(float(val)) >= int(float(field))
    except:
        return True
...
FILTER_FUNCS = {
    FILTER_CHOICE_LARGER_THAN:
        lambda val, field: tryLarger(val, field),
    FILTER_CHOICE_SMALLER_THAN:
        lambda val, field: trySmaller(val, field),
    FILTER_CHOICE_SORT_UP:
        lambda val, field: int(val).sort(),
    FILTER_CHOICE_SORT_DOWN:
...
number_filter_field = forms.ChoiceField(label=" ", required=False,
...
class EntriesForm(forms.Form):
    ...
    def __init__(self, form, request, formentry_model=FormEntry,
        ...
        for field in self.form_fields:
            ...
            elif field.is_a(*fields.NUMBERS):
                contains_field = forms.CharField(label=" ", required=False)
                self.fields["%s_filter" % field_key] = number_filter_field
                self.fields["%s_contains" % field_key] = contains_field
            else:
